...
function renderStudentWorks(
  ...
  const sortedWorks = [...works].sort(
    (firstWork, secondWork) =>
      new Date(`${secondWork.submitted}T12:00:00`) -
      new Date(`${firstWork.submitted}T12:00:00`),
  );
  ...
  if (!sortedWorks.length) {
    list.innerHTML =
      '<p class="works-empty">No projects have landed yet. Check back soon.</p>';
    pagination.hidden = true;
    return;
  }

  list.innerHTML = pageWorks
    .map((work, index) => {
      ...
      if (images.length > 1) {
        galleryControls =
          '<button class="work-gallery__arrow work-gallery__arrow--previous" type="button" aria-label="Previous image">&lt;</button><button class="work-gallery__arrow work-gallery__arrow--next" type="button" aria-label="Next image">&gt;</button><div class="work-gallery__dots">' +
          dots +
          "</div>";
      }
      let link =
        '<span class="work-card__unlinked">No showcase link yet</span>';
      if (work.link) {
  ...
  renderPagination(pagination, currentPage, pageCount, (nextPage) =>
    renderStudentWorks(sortedWorks, false, nextPage),
  );
  if (shouldSetupSearch) {
    setupSearch(sortedWorks, (matchingWorks) =>
      renderStudentWorks(matchingWorks, false, 1),
    );
  }
}
...
function setupGallery(card) {
  ...
  const showSlide = (nextSlide) => {
    ...
    images.forEach((image, index) =>
      image.classList.toggle("is-active", index === activeSlide),
    );
  ...
  card
    .querySelector(".work-gallery__arrow--previous")
    ?.addEventListener("click", () => showSlide(activeSlide - 1));
  card
    .querySelector(".work-gallery__arrow--next")
    ?.addEventListener("click", () => showSlide(activeSlide + 1));
  dots.forEach((dot, index) =>
