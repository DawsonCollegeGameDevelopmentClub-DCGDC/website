...
function renderSearchResult({ work }) {
  const image = work.images?.[0]
    ? `../assets/gameImages/build${work.id}/${work.images[0]}`
    : null;
  let imageHTML =
...
function setupSearch(works, onSearch) {
  ...
  function updateDropdown() {
    ...
    try {
      ...
      results.innerHTML =
        '<p class="search-results__message">Invalid regex pattern.</p>';
      return false;
    }

    const studentMatches = works
      .map((work) => ({
        work,
  ...
  function search() {
    ...
    const rankedMatches = getRankedMatches(works, regex, query).map(
      (result) => result.work,
    );
